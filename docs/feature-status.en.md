...
| `GET /v1/models` 接口 | ✅  | 按当前可用账号、账号类型和上游模型目录动态返回；图片模型仅在对应账号能力可用时出现。 |
...
| `/v1/complete` 文本补全与流式输出 | ❌  | 当前路由未提供；请使用 `/v1/chat/completions`。 |
| 流式输出支持 | ✅  | 已实现。 |
| 文本补全缓存与重复请求合并 | ✅  | `/v1/chat/completions` 文本链路默认启用 60 秒短缓存；非流式重复请求支持 in-flight 合并，已完成的流式结果支持回放，流式并发请求为避免占满 AI worker 各自执行；同时清理相邻重复消息。可通过 `chat_completion_cache` 配置关闭或调整。 |
| 图片尺寸参数 | ✅  | 图片生成与编辑接口按当前支持的尺寸集合校验并透传。 |
| 服务端图片 URL 缓存 | ✅  | 已实现。 |
| `rt_token` 刷新 | ❌  | 待实现。 |
| 代理配置功能 | ✅  | 已支持网页端配置全局 HTTP / HTTPS / SOCKS5 / SOCKS5H 代理，并应用到出站请求。 |
| Anthropic Messages 协议支持 | ⚠️  | 已提供 `POST /v1/messages` 的文本与流式兼容链；当前只支持代码中明确映射的字段，无法兑现的 Anthropic 参数会返回 400，不宣称完整协议覆盖。 |
