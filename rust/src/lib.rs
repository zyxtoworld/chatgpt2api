...
fn normalize_chat_effort(object: &mut Map<String, Value>) {
...
fn validate_chat_payload(payload: Value) -> Result<Map<String, Value>, ApiError> {
...
fn native_message(message: &Value) -> Result<Value, ApiError> {
...
fn native_conversation_payload(object: &Map<String, Value>) -> Result<Value, ApiError> {
    if object
        .get("tools")
        .and_then(Value::as_array)
...
fn native_message_text(message: &Value) -> Result<String, ApiError> {
    let object = message.as_object().ok_or_else(ApiError::invalid_request)?;
    let content = object
        .get("content")
        .ok_or_else(ApiError::invalid_request)?;
...
fn native_token_count(bpe: &tiktoken_rs::CoreBPE, text: &str) -> Result<usize, ApiError> {
    bpe.count(text, &HashSet::new())
        .map_err(|_| ApiError::upstream())
}

fn native_usage(object: &Map<String, Value>, output: &str) -> Result<Value, ApiError> {
    let model = object
        .get("model")
        .and_then(Value::as_str)
        .unwrap_or("auto");
...
fn native_usage_for_prompt_tokens(
    model: &str,
    prompt_tokens: usize,
    output: &str,
...
fn native_patch_candidate(
    ...
    if path == Some("/message/content/parts/0") {
        ...
        return match operation {
            ...
            // Python strips request-owned history before applying replace. The
            // isolated Rust canary does not carry that history owner, so it
            // must not publish a potentially stale replacement.
...
fn native_sanitize_text(text: &str) -> String {
...
fn native_usage_frame(completion_id: &str, model: &str, created: i64, usage: Value) -> Vec<u8> {
    let frame = json!({
        "id": completion_id,
        "object": "chat.completion.chunk",
        "created": created,
...
fn native_completion_text(body: &[u8]) -> Result<String, ApiError> {
    let mut text = String::new();
...
fn codex_client_version() -> Option<String> {
    env::var("CODEX_MODELS_CLIENT_VERSION")
        .ok()
        .and_then(|value| parse_codex_client_version(Some(&value)))
}

fn parse_codex_client_version(value: Option<&str>) -> Option<String> {
    let value = value?.trim();
...
fn native_codex_message(message: &Value) -> Result<Value, ApiError> {
    let object = message.as_object().ok_or_else(ApiError::invalid_request)?;
    let role = object
        .get("role")
        .and_then(Value::as_str)
...
fn native_codex_response_payload(object: &Map<String, Value>) -> Result<Value, ApiError> {
    if object
        .get("tools")
        .and_then(Value::as_array)
        .is_some_and(|tools| !tools.is_empty())
...
fn codex_sse_data(event: &[u8]) -> Result<Option<String>, io::Error> {
...
fn native_codex_text(body: &[u8]) -> Result<String, ApiError> {
    let mut buffer = body.to_vec();
    let mut text = String::new();
...
fn native_codex_input(value: &Value) -> Result<Value, ApiError> {
    let Value::Array(items) = value else {
...
fn native_codex_responses_payload(object: &Map<String, Value>) -> Result<Value, ApiError> {
    const ALLOWED_FIELDS: &[&str] = &[
        "model",
        "input",
        "instructions",
...
fn native_codex_responses_json(body: &[u8], requested_model: &str) -> Result<Value, ApiError> {
    let mut buffer = body.to_vec();
    let mut text = String::new();
