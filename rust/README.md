...
当前源码按职责拆分为 `config.rs`（配置与初始化错误）、`account_pool.rs`（账号快照、租约与并发池）、`model_pool.rs`（模型快照、目录刷新与公开投影）、`errors.rs`（公共错误映射）、`shutdown.rs`（服务启动与有界关闭）、`protocol_chat.rs`（Chat 输入校验与 reasoning 归一化）、`protocol_responses.rs`（Responses 输入校验与文本适配）、`protocol_codex_payload.rs`（Codex Responses 工具、输入历史与请求 payload 适配）、`codex_sse.rs`（Codex SSE 事件边界与 Chat frame）、`codex_upstream.rs`（Codex 版本、请求头与无状态 payload 适配），`lib.rs` 保留路由组合和尚未拆出的协议实现。拆分只改变模块归属，不改变运行时合同；每次拆分都必须通过独立 CI 的 fmt、Clippy `-D warnings` 和 `cargo test --locked --all-targets`。
