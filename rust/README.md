...
`RUST_AUTH_KEYS_PATH`、`RUST_MODELS_PATH` 与 `RUST_ACCOUNTS_PATH` 只能指向人工导出的隔离快照，严禁指向生产热文件。Rust canary 对 auth、模型和账号快照提供有界的 generation-aware reload：每次请求最多允许一个受控读取，并发请求等待当前 leader 完成；合法且指纹变化的完整快照才会原子替换当前内存代，坏代清空并 fail-closed，下一份合法快照才能恢复。账号格式为 `[{"access_token","status","type","source_type","chatgpt_account_id","models"}]` 或 `{"items":[...]}`，缺失 `type` 默认 `free`，缺失 `source_type` 默认 `web`，显式值会先规范化；旧 `oauth_login` 记录归一为 Codex 能力；公开目录仍按归一化 `account_type` 合并，但刷新内部按 `(source capability, account_type)` 选择一个确定性代表、只访问该 source 兼容端点，失败才在同一 source/type 候选中回退；`chatgpt_account_id` 只作为认证目录请求和同一 Codex lease 的可选身份头；在途租约保留自己的不可变 slot 直到请求结束，不会被新代 Drop 误释放；这不是 Python 生产账号/模型权威热 reload、token rotation 或撤销合同。
