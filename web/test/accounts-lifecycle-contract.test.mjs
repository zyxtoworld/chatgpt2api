...
test("account writes use one mutation owner and guard late results", () => {
  ...
  for (const handler of ["handleDeleteTokens", "handleRefreshAccounts", "handleUpdateAccount"]) {
...
test("the access-token-only import dialog exposes no OAuth flow", async () => {
  const text = readFileSync(importDialogPath, "utf8");
  assert.doesNotMatch(text, /oauthStartOwnerRef|startOAuthLogin|finishOAuthLogin|OAuth/);
});
