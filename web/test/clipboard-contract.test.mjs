...
test("account rows display a compact token preview while copying the complete token", () => {
  const displayIndex = accountsSource.lastIndexOf("{maskToken(account.access_token)}");
  assert.ok(displayIndex >= 0, "account rows must display a token preview");
  assert.match(accountsSource, /copyText\(account\.access_token, "token 已复制"\)/);
  assert.match(accountsSource, /max-w-\[13rem\] truncate font-mono/);
