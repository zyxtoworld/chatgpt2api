...
function hasForbiddenTokenFields(value) {
...
function getSub2ApiAccount(value) {
  ...
  if (hasForbiddenTokenFields(credentials) || hasForbiddenTokenFields(raw)) {
    return null;
  }
  ...
  const token = getString(credentials.access_token);
  if (!token) {
    return null;
  }
...
function getAccountJsonAccount(value) {
  ...
  if (hasForbiddenTokenFields(raw)) {
    return null;
  }
  ...
  const payload = {
    ...Object.fromEntries(Object.entries(raw).filter(([key]) => !["accessToken", "token", "refresh_token", "id_token"].includes(key))),
    access_token: token,
    source_type: "codex",
  };
