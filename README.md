...
```bash
git clone git@github.com:zyxtoworld/chatgpt2api.git
cd chatgpt2api
...
启动前请将宿主机 `config.json` 中的 `auth-key` 换成真实值，或通过 `CHATGPT2API_AUTH_KEY` 环境变量提供。这里的 `auth-key` 只负责登录 chatgpt2api 管理面板，与号池的 ChatGPT `access_token` 是两套独立凭据；号池不接受、不保存 `id_token` 或 `refresh_token`。`config.json` 只作为外置挂载进入容器，不会被构建进镜像；当前 `latest` 镜像的唯一后端入口是 Rust release 二进制。Compose 使用固定的 `/app/config.json` 和 `/app/data`，不需要额外的配置目录或路径环境变量。
...
```bash
cp .env.example .env
docker compose -f docker-compose.warp.yml pull app
...
Rust 主服务直接读取 `.env` 中的代理运行时默认值并在内存中合并，不再启动额外的配置初始化容器，也不会在启动时改写外置 `config.json`。默认只让上游 OpenAI / ChatGPT 请求走稳定代理，账号邮箱、CPA 等辅助链路不会被强制接管。账号自身配置的代理优先级最高，其次是稳定代理运行时，再其次是显式代理和旧版全局代理。
...
```bash
cargo run --bin chatgpt2api-rust
```
...
后端唯一实现是 Rust。默认 Rust 测试不访问真实 ChatGPT 上游，使用本地 fake transport 覆盖服务生命周期、格式转换、SSE 终态、工具、网页搜索、存储和迁移锁合同；Web 合同测试使用 Node/Bun。
...
cd web && node --test test/*.test.mjs && bun x tsc --noEmit
```

真实上游验收必须使用隔离配置、显式 Bearer 密钥和独立数据目录；缺少任一前置条件时直接 fail-closed，不把 HTTP 502 或只打印响应当成通过。
...
- 支持搜索、筛选、导出、手动编辑和清理账号
- 支持本地账号 JSON（项目、CPA、Sub2API 导出）、远程 CPA/Sub2API 服务器、ccLoad Codex 渠道和直接 `access_token` 导入
- Sub2API/ccLoad 导入只接受 `access_token`；refresh token、id token 和 OAuth 自动刷新均不支持
...
| `user`               | 当前上游没有终端用户标识透传能力，传入时明确返回 `400`                              |
| `client_task_id`     | 同步 `/v1/images/edits` 与异步 `/api/image-tasks/edits` 均支持受约束的任务标识，并在响应中回传 |
...
设置页中的 `default_thinking_effort` 会在请求未显式给出思考强度时生效；显式 `reasoning.effort`、`reasoning_effort` 或 `thinking_effort` 优先。图片 Web 链路的 `default_upstream_model_name` 可使用 `-standard`、`-extended` 或 `-max` 后缀覆盖默认强度，配置保存后下一请求直接读取外置 `config.json`。
...
<a href="https://github.com/zyxtoworld/chatgpt2api/graphs/contributors">
  <img alt="Contributors" src="https://contrib.rocks/image?repo=zyxtoworld/chatgpt2api" />
</a>

## Star History

[![Star History Chart](https://api.star-history.com/chart?repos=zyxtoworld/chatgpt2api&type=date&legend=top-left)](https://www.star-history.com/?repos=zyxtoworld%2Fchatgpt2api&type=date&legend=top-left)
