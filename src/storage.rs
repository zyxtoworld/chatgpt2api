...
"#,
        ...
        let mut background_import = tokio::spawn({
            let state = state.clone();
            async move {
                state
                    .account_store
                    .merge_import_records(vec![serde_json::json!({
                        "access_token": "git-publish-new-account-a",
                        "type": "pro",
                        "source_type": "web"
                    })])
                    .await
        ...
        assert!(
            tokio::time::timeout(Duration::from_millis(100), &mut background_import)
                .await
                .is_err(),
            "background account import crossed the Phase B account gate"
        );
        ...
        assert_eq!(state.account_store.records()[0].status, "正常");
        ...
        assert_eq!(published["accounts"]["abnormal"], 0);
