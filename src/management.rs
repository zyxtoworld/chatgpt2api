...
async fn load_ccload_channel_models(
    ...
    let mut model_tokens = Vec::<(usize, String, Option<String>, Option<String>)>::new();
    ...
    for (catalog_index, access, plan_type, account_id) in model_tokens {
    ...
    while let Some((catalog_index, Some(model_ids))) = requests.next().await {
