...
use super::{
    ...
    acquire_native_codex_lease, codex_sse_data, config, native_codex_responses_payload,
...
fn credential_key(lease: &super::AccountLease) -> CredentialKey {
    let raw_proxy = lease
        .proxy_url()
        .map(ToOwned::to_owned)
        .or_else(configured_upstream_proxy)
        .unwrap_or_default();
    CredentialKey {
...
fn configured_upstream_proxy() -> Option<String> {
    let explicit = env::var("RUST_UPSTREAM_PROXY").ok();
    let runtime = config::proxy_runtime_defaults_from_environment();
    upstream_proxy_from_runtime(&runtime, explicit.as_deref())
}

fn upstream_proxy_from_runtime(runtime: &Value, explicit: Option<&str>) -> Option<String> {
    ...
    let proxy_url = runtime
        ...
        .map(str::trim)
        .filter(|value| !value.is_empty())?;
    (enabled && egress_mode == "single_proxy").then(|| proxy_url.to_owned())
}
...
mod tests {
    ...
    #[test]
    fn runtime_proxy_environment_requires_enabled_single_proxy_mode() {
        let runtime = json!({
            "enabled": true,
        ...
        assert_eq!(
            upstream_proxy_from_runtime(&runtime, None).as_deref(),
            Some("http://privoxy:8118")
        );
        assert_eq!(
            upstream_proxy_from_runtime(
                &json!({
                    "enabled": false,
                    "egress_mode": "single_proxy",
                    "proxy_url": "http://privoxy:8118",
        ...
        assert_eq!(
            upstream_proxy_from_runtime(&runtime, Some("http://explicit:8080")).as_deref(),
